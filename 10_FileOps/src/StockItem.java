...
public class StockItem {
	...
	public void setRecordLength() {
		// Integer, Long and Double sizes are in bits therefore we must divide by 8 to
		// obtain bytes.
		// Java uses wide characters which contain two bytes therefore we must multiply
		// DESCRIPTION_LENGTH by 2.
		recordLength = (Integer.SIZE / 8) + (Long.SIZE / 8) + (DESCRIPTION_LENGTH * 2) + (Double.SIZE / 8)
				+ (Integer.SIZE / 8);
	}
	...
	public String toString() {
		return "Record status   : " + Integer.toString(getRecordStatus()) + "\n" + "Item number     : "
				+ Long.toString(this.getItemNumber()) + "\n" + "Description     : " + this.getDescription() + "\n"
				+ "Cost            : " + Double.toString(this.getCost()) + "\n" + "Quantity on-hand: "
				+ Integer.toString(this.getQuantityOnHand()) + "\n";
	}
